...
- 歌词、逐字歌词、翻译、罗马音与封面地址。
- 跨平台歌曲匹配。
- 兼容 LX 自定义源 API v2 的多脚本 `musicUrl` 解析、同平台优先选择与故障回退。
- 返回短期直链、代理音频流、服务端持久化下载任务，并拒绝明显过短的试听/防盗链占位音频。
- 单一 Bearer API 密钥、CORS 白名单、速率限制、路由截止时间、请求中断与上游 URL 安全检查。
- SQLite 任务持久化；服务重启后，未完成任务转为 `paused`，不会擅自继续。

不包含播放器、本地曲库、“我的列表”、同步、GUI/Electron，以及实验性的歌手、专辑或搜索建议接口。自定义源只能由 `config.toml` 指定显式脚本或本地目录；没有上传、切换、查看或下载源脚本的 API。
...
3. 在 `custom_source.script_path` 指定单个脚本，或将其留空并通过 `custom_source.directory_path` 指定脚本目录。目录第一层的全部 `.js` 会自动加载；建议放在已忽略的 `.private/` 中。
...
没有任何自定义源通过校验时，服务仍会启动：`/readyz` 返回 HTTP 200 和 `degraded`。只要至少一个脚本可用，解析能力即保持就绪；搜索、歌单、歌词等官方平台能力不受失效脚本影响。
...
最后一条只在本机读取配置指定的全部自定义源并汇总验证初始化能力，不会启动 API，也不会提交源脚本。`config.toml`、`.private/`、运行数据库和下载目录都已加入 Git 忽略。
